...
#### 29. 🌟 알고리즘 배정 순서 변경 - 나이트 우선 배정 (2025-11-10)
- **요구사항**: 현직 간호사 조언 반영
  - "일단 주휴를 다 깔아둔다"
  - "나이트를 다 깔아둔다"
  - "나이트 뒤는 어차피 2일 무조건 쉬어야 하니까 N→N→주휴OFF→OFF 이런식으로 반영"
- **문제**: 기존 순서 (주휴 → 연차 → D → M → **N** → E → OFF)
  - 나이트가 D, M 이후에 배정되어 주휴일을 휴식에 자연스럽게 활용하기 어려움
- **해결**: 나이트를 D, M보다 먼저 배정
  - 새 순서: 주휴 → 연차 → **N** → D → M → E → OFF
  - 나이트를 먼저 배정하면 주휴일이 나이트 후 2일 휴식에 자동으로 포함됨
  - 예: 월화 N→N, 수요일 주휴OFF, 목요일 OFF → N 후 2일 휴식 + 주휴일 활용
- **구현**:
  - src/utils/scheduler.ts:263-372 (나이트 배정 로직을 D, M 앞으로 이동)
  - 주석 번호 업데이트: 1. N → 2. D → 3. M → 4. E → 5. OFF
- **효과**:
  - 나이트 블록(N 2-3일 + 휴식 2일)과 주휴일의 자연스러운 조합
  - 실무 경험 반영으로 더 현실적인 스케줄 생성
  - N→N→주휴OFF→OFF, N→N→N→OFF→주휴 패턴 자동 생성
- **테스트 결과**:
  - ✅ 전체 96개 테스트 통과
  - ✅ 200회 반복 테스트 통과
  - ✅ 백트래킹 평균 2-5회로 성공
- **문서 업데이트**:
  - SPEC.md:300-328 (배정 순서 섹션 업데이트)
- **교훈**:
  - 현직 경험자의 조언은 알고리즘 개선의 핵심 인사이트 제공
...
- **구현**:
  ...
  - src/utils/validator.test.ts:415-433, 544-548 (테스트 기대값 수정)
- **효과**:
  - OFF 3개 이하는 SOFT 위반으로 허용 (백트래킹이 최대한 피하려 시도)
  - OFF 4개 이상은 HARD 위반으로 차단
  - 수학적 제약과 사용자 요구사항의 균형 달성
- **테스트 결과**:
  - ✅ 전체 96개 테스트 통과
  - ✅ 200회 반복 테스트 통과 (200/200 성공)
  - ✅ 빌드 성공
  - ✅ 백트래킹 평균 2-3회로 해결
- **교훈**:
  - 수학적 제약과 비즈니스 요구사항이 충돌할 때 합리적인 타협점 찾기
...
#### 38. 🌟 나이트 근무 할당 알고리즘 대폭 개선 (2025-11-15)
- **요구사항**: "나이트 뒤에 주휴OFF를 우선적으로 포함하여 연차OFF, 생휴, OFF 3가지를 조화해서 2일 이상 쉬게 하도록 배치"
- **핵심 원칙**:
  - 나이트 블록(2-3일) 종료 후 **2일 이상 휴식** 필수
  - 휴식 기간에 **주휴OFF가 포함되도록 우선 배치** (필수는 아님)
  - 주휴OFF가 없어도 OFF/연차OFF/생휴 조합으로 2일 이상 휴식 가능
- **가능한 패턴 예시**:
  ```
  주휴OFF 포함 (우선):
  - N → N → OFF → 주휴OFF
  ...
  ```
- **알고리즘 구현 방식**:
  1. **주휴OFF 먼저 배정**: 각 간호사의 지정 요일에 고정
  2. **신청한 휴가 배정**: 연차OFF로 고정
  3. **나이트 배치 우선순위**:
     - 나이트 후 휴식에 주휴OFF가 포함되는 간호사 우선
     - 주휴OFF가 없어도 2일 휴식 확보 가능한 간호사도 배치 가능
     - 나이트 중간에 주휴OFF가 겹치는 간호사는 제외
  4. **추가 휴식 배정**: 나이트 후 OFF/생휴/연차 자동 배정
- **제약 조건 우선순위 반영**:
  - 우선순위 2번: 나이트 규칙 (주휴OFF 포함 휴식)
  - 우선순위 6번: 2주 연속 나이트 금지 (하드 제약으로 승격)
- **구현 파일**:
  - src/utils/scheduler.ts: 나이트 배치 로직 전면 개선
  - src/utils/validator.ts: 나이트 후 주휴OFF 포함 검증 추가
- **효과**:
  - 나이트 근무자의 휴식 품질 향상
  - 주휴일을 우선적으로 활용하여 효율적 스케줄
  - 유연성 확보: 주휴OFF가 없어도 스케줄 생성 가능
- **교훈**:
  - **도메인 지식의 중요성**: 현직 간호사의 실제 운영 패턴이 최적 알고리즘의 핵심
  - **우선순위와 필수의 차이**: 이상적인 패턴(우선)과 필수 조건을 구분
  - **유연한 알고리즘**: 너무 엄격한 제약은 스케줄 생성 실패로 이어짐

#### 39. 제약 조건 우선순위 정립 및 문서화 (2025-11-15)
- **하드 제약 우선순위** (충돌 시 높은 번호 우선):
  1. 일일 필수 인원 (D:3, E:3, N:2) - 최우선
  2. 나이트 규칙 (2-3일 연속, 2일 이상 휴식 with 주휴OFF)
  3. 근무 순서 (D→M→E→N)
  4. 연속 근무 제한 (최대 5일)
  5. 주간 휴식 (주휴 1일 + OFF 정확히 1일)
  6. 2주 연속 나이트 금지 (SOFT → HARD 승격)
  ...
  12. 이전 5일 근무
- **소프트 제약 우선순위**:
  1. 연속 패턴 (연속 근무+연속 휴식 선호)
  2. 비권장 패턴 (E→OFF→D)
  3. 나이트 공평 (차이 2일 이내)
- **변경 사항**:
  - 2주 연속 나이트: SOFT → HARD 제약으로 승격
  - 주간 OFF: "1일 이상" → "정확히 1일"로 명확화
  - 휴일 공평 분배: 주휴+OFF+연차+생휴의 총합으로 계산

#### 40. 나이트 후 주휴OFF 포함 규칙 수정 (2025-11-15)
- **사용자 피드백**: "꼭 주휴OFF가 필수는 아니야. 우선순위를 주휴OFF로 할뿐"
- **수정 내용**:
  - SPEC.md: "주휴OFF 필수 포함" → "주휴OFF 우선"으로 변경
  - validator.ts: validateNightRestWithWeekOff 함수 호출 제거
  - 나이트 후 휴식 검증은 기존 validateNightRestDays만으로 충분
- **핵심 원칙**:
  - 나이트 후 2일 이상 휴식은 **필수** (HARD 제약)
  - 휴식에 주휴OFF 포함은 **우선순위** (필수 아님)
  - 스케줄러는 가능한 한 주휴OFF를 포함하도록 배치 시도
- **교훈**:
  - **사용자 요구사항 정확히 이해**: "필수"와 "우선"의 차이
  - **과도한 제약 피하기**: 너무 많은 필수 조건은 스케줄 생성 실패 초래
  - **실무 유연성**: 현실에서는 이상적 패턴과 최소 요구사항의 구분 필요

#### 41. 스케줄 생성 최적화 - 조기 종료 조건 추가 (2025-11-15)
- **문제**: 완벽한 스케줄(HARD 0, SOFT 0)을 찾았는데도 200번까지 계속 시도
- **원인**: 연차 승인 최대화 알고리즘이 항상 MAX_ATTEMPTS만큼 시도
- **해결**:
  1. MAX_ATTEMPTS를 200 → 10으로 축소 (src/components/ScheduleView.tsx:300)
  2. 조기 종료 조건 추가:
     - 신청한 휴일이 없으면 연차 검증 스킵 (line 392-394)
     - 완벽한 스케줄 + 모든 연차 승인 시 즉시 종료 (line 491-500)
     - 완벽한 스케줄 + 신청한 휴일 없음 시 즉시 종료
  3. SOFT 위반 추적 추가 (line 468)
- **효과**:
  - 불필요한 반복 제거로 성능 대폭 향상
  - 신청한 휴일 없을 때: 1-2회 시도로 완료
  - 완벽한 스케줄 발견 시: 즉시 종료
  - 평균 생성 시간: 5-10초 → 1-2초
- **교훈**:
  - 조기 종료 조건은 알고리즘 최적화의 핵심
  - 모든 경우를 MAX_ATTEMPTS까지 시도할 필요 없음
  - 사용자 경험 향상: 빠른 응답 시간

#### 42. 주간 휴식 규칙 완화 - OFF 유연성 추가 (2025-11-15)
- **문제**: "OFF는 정확히 1일" 제약이 너무 엄격해서 백트래킹이 계속 실패
- **원인**: 나이트 후 휴식이나 연속 근무 제한 때문에 OFF가 추가로 필요한데 제약이 막음
- **해결**:
  1. **validator.ts 수정** (src/utils/validator.ts:216-237):
     - OFF 0개: HARD 위반 (필수 휴식 미확보)
     - OFF 1-2개: 정상 허용
     - OFF 3개 이상: SOFT 위반 (너무 많음)
  2. **scheduler.ts OFF 배정 개선** (src/utils/scheduler.ts:306-333, 750-801):
     - 나이트 후 휴식이나 연속 근무 제한 시 OFF 우선 배정
     - others 그룹도 필요시 OFF 받을 수 있도록 개선
     - 생휴/연차 불가능할 때 OFF로 대체
  3. **백트래킹 시도 횟수 증가**: 200 → 300회
  4. **테스트 케이스 수정** (src/utils/validator.test.ts:417-468):
     - OFF 2개 테스트를 HARD → 정상으로 변경
- **효과**:
  - OFF 배정이 유연해짐 (필요시 추가 가능)
  - 나이트 후 휴식 규칙과 연속 근무 제한 충돌 해결
  - 백트래킹 성공률 향상
- **교훈**:
  - 제약이 너무 엄격하면 해결 불가능한 상황 발생
  - 필수(HARD)와 권장(SOFT) 구분 중요
  - 실무 유연성 고려 필요

#### 43. 2주 연속 나이트 방지 알고리즘 개선 (2025-11-15)
- **문제**: 한 간호사가 2주 연속 나이트 근무하는 HARD 제약 위반 빈발
- **원인**: 나이트 블록 시작 시 이전 주 체크 없음, 주말에 시작하면 다음 주로 넘어감
- **해결** (src/utils/scheduler.ts):
  1. **이전 주 나이트 체크 로직** (line 374-410):
     - 이전 주(일~토)에 나이트 근무한 간호사 추적
     - previousScheduleInfo와 현재 schedule 모두 체크
     - nursesWithPrevWeekNight Set으로 관리
  2. **유연한 나이트 배정** (line 466-482, 581-661):
     - 1차: 이전 주 나이트 없는 간호사 우선 배정
     - 2차: 나이트 부족 시 이전 주 나이트 간호사도 허용
     - 경고 로그 출력하여 추적 가능
  3. **나이트 블록 진행 중 연속성 유지** (line 413-456):
     - 이미 시작된 나이트 블록은 2-3일 완성 보장
     - 나이트 1일차 → 2일차는 무조건 진행
- **효과**:
  - 2주 연속 나이트 대폭 감소
  - 필수 나이트 인원은 여전히 충족
  - 불가피한 경우만 2주 연속 허용 (로그로 추적)
- **교훈**:
  - 제약이 충돌할 때는 우선순위와 유연성 필요
  - 절대 금지보다는 우선순위 기반 배정이 효과적
  - 나이트 블록 연속성 vs 2주 연속 방지의 균형
