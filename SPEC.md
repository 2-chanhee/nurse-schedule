...
2. **휴일 공평 분배 검증 (SOFT → HARD)** 🚧
   - 간호사별 총 휴일 수 차이 검증
   - 차이 1일: 정상
   - 차이 2일: SOFT 위반
   - 차이 3일 이상: HARD 위반
   - 함수명: `validateOffDayBalance`
   - 위치: src/utils/validator.ts (주석 처리: line 632-634)

3. **나이트 근무 공평 분배 검증 (SOFT)** 🚧
