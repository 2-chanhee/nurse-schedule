...
- **총 휴일**: 최소 2일 이상. 주휴 1일 + OFF 1일은 무조건 포함. 만약 연차나 생휴가 있다면 추가로 가능
...
| 타입      | 한글명 | 설명                   | 우선순위 | 색상 코드 |
| --------- | ------ | ---------------------- | -------- | --------- |
| D         | 데이   | 주간 근무              | 1        | #fbbf24 (밝은 노란색) |
| M         | 중간   | 중간 근무 (선택적)     | 2        | #ec4899 (진한 분홍색) |
| E         | 이브닝 | 저녁 근무              | 3        | #3b82f6 (진한 파란색) |
| N         | 나이트 | 야간 근무 (2-3일 연속) | 4        | #9333ea (진한 자주색) |
| OFF       | 오프   | 일반 휴무              | -        | #f97316 (진한 주황색) |
| WEEK_OFF  | 주휴   | 주휴일 (지정 요일)     | -        | #f97316 (진한 주황색) |
| ANNUAL    | 연차   | 연차 휴가              | -        | #f97316 (진한 주황색) |
| MENSTRUAL | 생휴   | 생리 휴가              | -        | #f97316 (진한 주황색) |

### UI 색상 설계 원칙

- **색약 고려**: 각 근무 타입이 명확히 구분되도록 대비가 강한 색상 선택
- **휴무 통일**: 모든 휴무 타입(OFF, WEEK_OFF, ANNUAL, MENSTRUAL)은 동일한 주황색 사용
- **나이트 vs 이브닝**: 자주색과 파란색으로 명확히 구분
- **가독성**: 모든 셀에 흰색 텍스트 사용으로 대비 확보

### 일일 필수 인원 카운트 상태 표시

| 상태    | 의미           | 색상 | 효과 |
| ------- | -------------- | ---- | ---- |
| error   | 필수 인원 부족 | 진한 빨간색 (#dc2626) + 흰색 텍스트 | 맥동 애니메이션 |
| warning | 필수 인원 초과 | 진한 주황색 (#f59e0b) + 흰색 텍스트 | 맥동 애니메이션 |
| ok      | 정상           | 연한 초록색 (#d1fae5) + 진한 초록 텍스트 | - |
...
2. **스케줄 테이블 UI**
   - 날짜별 간호사 근무 배정
   - 셀 클릭으로 근무 타입 순환 변경
   - 셀 표시: OFF, WO(주휴), A(연차), 생휴, D, M, E, N
   - 간호사별/날짜별 통계 표시
   - 주말 표시 (빨간색)
   - 1주 단위 구분선 (일요일 왼쪽에 굵은 세로선)

3. **자동 생성 기능**
   - 일일 필수 인원 충족 (D: 3, M: 1, E: 3, N: 2)
   - 근무 순서 규칙 준수 (D → M → E → N)
   - 휴일 후에는 어떤 근무든 시작 가능 (순서 초기화)
   - 공평한 근무 분배 (근무 횟수 기반)
   - 매번 다른 스케줄 생성 (랜덤 요소)
   - 재생성 버튼 (여러 번 생성 가능)
...
7. **UI 색상 개선** (색약 고려)
   - 근무 타입별 명확한 색상 구분
   - D: 밝은 노란색, M: 진한 분홍색, E: 진한 파란색, N: 진한 자주색
   - 모든 휴무 타입: 진한 주황색 통일
   - 일일 필수 인원 미충족/초과 시 강조 (맥동 애니메이션)
     - error(부족): 진한 빨간색 + 맥동
     - warning(초과): 진한 주황색 + 맥동
...
2. **주간 휴식 규칙 검증** (부분 완료)
   - ✅ 주휴일 1일 자동 배정 완료
   - 🚧 OFF + WEEK_OFF가 1주 내에 무조건 1개씩 포함되어야 함(총 2일)
...
- 간단한 자동 스케줄 생성
- 일일 필수 인원 충족 (최우선)
- 근무 순서 준수 (D → M → E → N, 휴일 후 초기화)
- 공평한 근무 분배 (근무 카운트 기반, 랜덤 요소)
- 연속 근무일 제한 준수 (최대 5일)

**알고리즘 순서:**
0. 주휴일 배정 (최우선) - 간호사별 지정 요일에 WEEK_OFF 자동 배정 (isFixed: true)
1. 연속 근무일 체크 - 5일 연속 근무한 간호사는 근무 배정 제외
2. 나이트 근무 배정 (2명) - 휴일 후, E, N 다음 또는 첫날
3. 데이 근무 배정 (3명) - 휴일 후, D 연속 또는 첫날
4. 중간 근무 배정 (1명) - 휴일 후, D, M 다음 또는 첫날
5. 이브닝 근무 배정 (3명) - 휴일 후, D, M, E 다음 또는 첫날
6. 나머지 OFF 배정
7. 연속 근무일 자동 업데이트 (근무시 +1, 휴일시 리셋)

**배정 규칙:**
- 휴일(OFF, WEEK_OFF, ANNUAL, MENSTRUAL) 후에는 어떤 근무든 시작 가능
- 근무 순서: D → M → E → N (건너뛰기 가능, 역순 불가)
- 랜덤 요소: 근무 카운트 같을 때 랜덤 정렬로 매번 다른 스케줄 생성
